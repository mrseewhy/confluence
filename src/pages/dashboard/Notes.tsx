    ...
    <DashboardLayout user={user} variant="user">
      ...
      {filtered.length > 0 ? (
        <div style={{ background: 'var(--color-bg-elevated)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-xl)', overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 160px 90px 120px 140px', gap: 'var(--space-4)', padding: 'var(--space-3) var(--space-5)', borderBottom: '1px solid var(--color-border)', background: 'var(--color-bg-subtle)' }}>
          ...
          {filtered.map((note, i) => (
            <div key={note.id} style={{
              display: 'grid', gridTemplateColumns: '1fr 160px 90px 120px 140px',
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', minWidth: 0 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <p style={{ margin: 0, fontSize: 'var(--font-size-sm)', fontWeight: 'var(--font-weight-semibold)', color: 'var(--color-text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {note.title}
                  </p>
                  ...
                  {note.visibility === 'public' ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', marginTop: '2px' }}>
                      <span style={{ fontSize: '10px', color: 'var(--color-accent)', fontFamily: 'var(--font-mono)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {`/n/${note.slug}`}
                      </span>
                      <button 
        ...
        <EmptyState icon="📝" title="No notes found" description="Create your first note to get started." action={
          <Link to="/dashboard/notes/new">
            <Button variant="primary" size="sm" leftIcon={<Icon d={IC.plus} size={14} />}>New note</Button>
          </Link>
        } />
