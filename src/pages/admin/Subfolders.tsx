...
// ── Subfolder Selector (shared across action cards) ──────────

function SubfolderSelect({
  folders,
  selectedId,
  onSelect,
  placeholder,
}: {
  folders: SubfolderRow[];
    ...
    <div ref={ref} style={{ position: "relative", width: "100%" }}>
      <button type="button" onClick={() => setOpen((p) => !p)}
        ...
        <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>
          {selected ? `${selected.title}${selected.parent ? ` → ${selected.parent.title}` : ""}` : placeholder || "Select a subfolder…"}
        </span>
          ...
          <div style={{ overflowY: "auto", flex: 1 }}>
            {filtered.length > 0 ? filtered.map((f) => (
              <button key={f.id} type="button" onClick={() => { onSelect(f.id); setOpen(false); setQuery(""); }}
                style={{ display: "block", width: "100%", padding: "var(--space-2) var(--space-3)", border: "none", background: f.id === selectedId ? "var(--color-accent-subtle)" : "transparent", color: "var(--color-text-primary)", fontSize: "var(--font-size-sm)", textAlign: "left", cursor: "pointer", fontFamily: "var(--font-sans)" }}
...
export function AdminSubfolders() {
  ...
  const loadSubfolders = async () => {
    try {
      ...
      if (folders) setSubfolders(folders);
    } catch (err) {
  ...
  const filtered = subfolders.filter((f) => {
    ...
    return (
      (f.title.toLowerCase().includes(q) || (f.description ?? "").toLowerCase().includes(q) || (f.parent?.title ?? "").toLowerCase().includes(q)) &&
  ...
  const getFolder = (id: string | null) => subfolders.find((f) => f.id === id);
    ...
    <DashboardLayout user={user} variant="admin">
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "var(--space-4)", flexWrap: "wrap", gap: "var(--space-3)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
              ...
              <span style={{ fontSize: "var(--font-size-xs)", color: "var(--color-text-muted)" }}>
                {filtered.length} of {subfolders.length} · Page {page} of {totalPages}
              </span>
            </div>
        ...
        {paginated.length > 0 ? (
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 120px 60px 90px 100px 80px", gap: "var(--space-3)", padding: "var(--space-3) var(--space-5)", borderTop: "1px solid var(--color-border)", borderBottom: "1px solid var(--color-border)", background: "var(--color-bg-subtle)" }}>
              {["Subfolder", "Parent", "Vis.", "Updated", "Owner", ""].map((h) => (
            ...
            {paginated.map((folder, i) => (
              <div key={folder.id} style={{ display: "grid", gridTemplateColumns: "1fr 120px 60px 90px 100px 80px", gap: "var(--space-3)", alignItems: "center", padding: "var(--space-3) var(--space-5)", borderBottom: i < paginated.length - 1 ? "1px solid var(--color-border-subtle)" : "none", transition: "background var(--duration-fast)" }}
      ...
      <div className="action-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--space-5)" }}>
        ...
        <ActionPanel title="Toggle Visibility" description="Switch a subfolder between public and private" icon={IC.globe} accent="accent">
          <SubfolderSelect folders={subfolders} selectedId={visSelectedId} onSelect={setVisSelectedId} placeholder="Choose a subfolder…" />
          {visSelectedId && (() => {
        ...
        <ActionPanel title="Transfer Ownership" description="Reassign a subfolder to a different user" icon={IC.users} accent="warning">
          <SubfolderSelect folders={subfolders} selectedId={transferItem?.id || null} onSelect={(id) => {
            const f = getFolder(id);
            if (f) setTransferItem({ id: f.id, title: f.title, ownerName: f.owner?.full_name || null });
          }} placeholder="Choose a subfolder to transfer…" />
          {transferItem && (
                  ...
                  try {
                    ...
                    void (async () => {
                      try { await supabase.from("activity_log").insert({
                        ...
                        details: `Admin ${user?.full_name} transferred subfolder "${transferItem.title}" to ${newOwnerName}`,
                      }); } catch {}
                    })();
