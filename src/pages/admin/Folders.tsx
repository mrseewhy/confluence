...
export function AdminFolders() {
  ...
  const loadFolders = async () => {
    try {
      ...
      // Load collaborators for all folders
    ...
    <DashboardLayout user={user} variant="admin">
      ...
      {filtered.length > 0 ? (
        <div style={{ background: "var(--color-bg-elevated)", border: "1px solid var(--color-border)", borderRadius: "var(--radius-xl)", overflow: "hidden" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 60px 70px 80px 90px 100px 170px", gap: "var(--space-3)", padding: "var(--space-3) var(--space-5)", borderBottom: "1px solid var(--color-border)", background: "var(--color-bg-subtle)" }}>
            {["Folder", "Type", "Vis.", "Collabs", "Updated", "Owner", "Actions"].map((h) => (
          ...
          {filtered.map((folder, i) => {
            ...
            return (
              <div key={folder.id} style={{ display: "grid", gridTemplateColumns: "1fr 60px 70px 80px 90px 100px 170px", gap: "var(--space-3)", alignItems: "center", padding: "var(--space-4) var(--space-5)", borderBottom: i < filtered.length - 1 ? "1px solid var(--color-border-subtle)" : "none", transition: "background var(--duration-fast)" }}
