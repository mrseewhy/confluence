...
function folderToSelect(f: FolderRow): SelectItem {
  return {
    id: f.id,
    primary: f.title,
    secondary: f.owner?.full_name || undefined,
...
export function AdminFolders() {
  ...
  const loadFolders = useCallback(async (currentSearch: string, currentVis: "all" | "public" | "private", currentPage: number) => {
    setLoading(true);
    try {
      ...
      let query = supabase
        .from("folders")
        .select("*, owner:profiles(full_name)", { count: "exact" });
  ...
  // When search/vis changes, reset to page 1 — the page state change triggers loadFolders
  // The first useEffect handles page reset, then the second triggers the load

  const getFolder = useCallback((id: string | null) => allFolders.find((f) => f.id === id), [allFolders]);
          ...
          <TransferOwnershipModal
            ...
            onTransfer={async (newOwnerId, newOwnerName, newOwnerEmail) => {
              try {
                ...
                await loadFolders(debouncedSearch, vis, page);
