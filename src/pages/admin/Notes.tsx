...
// ── Note Selector (shared across action cards) ───────────────
    ...
    <div ref={ref} style={{ position: "relative", width: "100%" }}>
      <button type="button" onClick={() => setOpen((p) => !p)}
        ...
        <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>
          {selected ? `${selected.title}${selected.folder ? ` → ${selected.folder.title}` : ""}` : placeholder || "Select a note…"}
        </span>
      ...
      {open && (
        <div style={{
          ...
          <input type="text" placeholder="Search…" value={query} onChange={(e) => setQuery(e.target.value)} autoFocus
            style={{ width: "100%", fontFamily: "var(--font-sans)", fontSize: "var(--font-size-sm)", color: "var(--color-text-primary)", background: "var(--color-bg-base)", border: "none", borderBottom: "1px solid var(--color-border)", padding: "var(--space-2) var(--space-3)", outline: "none", boxSizing: "border-box" }} />
          <div style={{ overflowY: "auto", flex: 1 }}>
            {filtered.length > 0 ? filtered.map((n) => (
              <button key={n.id} type="button" onClick={() => { onSelect(n.id); setOpen(false); setQuery(""); }}
                style={{ display: "block", width: "100%", padding: "var(--space-2) var(--space-3)", border: "none", background: n.id === selectedId ? "var(--color-accent-subtle)" : "transparent", color: "var(--color-text-primary)", fontSize: "var(--font-size-sm)", textAlign: "left", cursor: "pointer", fontFamily: "var(--font-sans)" }}
                onMouseEnter={(e) => (e.currentTarget.style.background = "var(--color-bg-muted)")}
                onMouseLeave={(e) => (e.currentTarget.style.background = n.id === selectedId ? "var(--color-accent-subtle)" : "transparent")}>
                <div style={{ fontWeight: "var(--font-weight-medium)" }}>{n.title}</div>
                <div style={{ fontSize: "10px", color: "var(--color-text-muted)", display: "flex", gap: "var(--space-2)" }}>
                  <span>{n.folder?.title || "—"}</span>
                  <Badge variant={n.visibility === "public" ? "accent" : "muted"} style={{ fontSize: "9px" }}>{n.visibility}</Badge>
                </div>
              </button>
            )) : (
...
export function AdminNotes() {
  ...
  const loadNotes = async () => {
    try {
      ...
      if (data) setNotesList(data as NoteRow[]);
    } catch (err) {
  ...
  const getNote = (id: string | null) => notesList.find((n) => n.id === id);
  ...
  if (!user || loading) {
    return (
      <DashboardLayout user={user || fallbackProfile({ user_type: "admin" })} variant="admin">
        <div style={{ padding: "var(--space-20)", textAlign: "center", color: "var(--color-text-muted)" }}>Loading notes…</div>
      </DashboardLayout>
    );
  }
  ...
  return (
    <DashboardLayout user={user} variant="admin">
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "var(--space-6)", flexWrap: "wrap", gap: "var(--space-4)" }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)", marginBottom: "var(--space-1)" }}>
            ...
            <h1 style={{ fontSize: "var(--font-size-2xl)", fontWeight: "var(--font-weight-bold)", letterSpacing: "var(--letter-spacing-tight)", margin: 0 }}>Note Management</h1>
          </div>
      ...
      <Card style={{ padding: 0, overflow: "hidden", marginBottom: "var(--space-8)" }}>
        <div style={{ padding: "var(--space-5) var(--space-5) 0" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "var(--space-4)", flexWrap: "wrap", gap: "var(--space-3)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
              <h2 style={{ fontSize: "var(--font-size-md)", fontWeight: "var(--font-weight-semibold)", margin: 0 }}>All Notes</h2>
              <span style={{ fontSize: "var(--font-size-xs)", color: "var(--color-text-muted)" }}>
                {filtered.length} of {notesList.length} · Page {page} of {totalPages}
              </span>
            </div>
        ...
        {paginated.length > 0 ? (
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 120px 60px 90px 100px 80px", gap: "var(--space-3)", padding: "var(--space-3) var(--space-5)", borderTop: "1px solid var(--color-border)", borderBottom: "1px solid var(--color-border)", background: "var(--color-bg-subtle)" }}>
              {["Note", "Folder", "Vis.", "Updated", "Owner", ""].map((h) => (
            ...
            {paginated.map((note, i) => (
              <div key={note.id} style={{ display: "grid", gridTemplateColumns: "1fr 120px 60px 90px 100px 80px", gap: "var(--space-3)", alignItems: "center", padding: "var(--space-3) var(--space-5)", borderBottom: i < paginated.length - 1 ? "1px solid var(--color-border-subtle)" : "none", transition: "background var(--duration-fast)" }}
      ...
      <div className="action-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--space-5)" }}>
        ...
        <ActionPanel title="Toggle Visibility" description="Switch a note between public and private" icon={IC.globe} accent="accent">
          <NoteSelect notes={notesList} selectedId={visSelectedId} onSelect={setVisSelectedId} placeholder="Choose a note…" />
          {visSelectedId && (() => {
                ...
                <Button variant={target.visibility === "public" ? "secondary" : "accent-ghost"} size="sm"
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setNotesList((prev) => prev.map((n) => n.id === target.id ? { ...n, visibility: newVis as "public" | "private" } : n));
        ...
        <ActionPanel title="Transfer Ownership" description="Reassign a note to a different user" icon={IC.users} accent="warning">
          <NoteSelect notes={notesList} selectedId={transferItem?.id || null} onSelect={(id) => {
            const n = getNote(id);
            if (n) setTransferItem({ id: n.id, title: n.title, ownerName: n.owner?.full_name || null });
          }} placeholder="Choose a note to transfer…" />
          {transferItem && (
